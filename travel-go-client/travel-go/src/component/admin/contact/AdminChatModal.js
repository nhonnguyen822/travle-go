...
const AdminChatModal = ({
            ...
            try {
                ...
                if (data) {
                    setCurrentUser({
                        ...authUser,
                        id: data.id,
                        role: data.role?.name || data.role || "ADMIN",
    ...
    // Init room + load messages - CHỈ SỬ DỤNG getAdminMessages
    useEffect(() => {
        ...
        const initChat = async () => {
            ...
            try {
                ...
                // Load messages - CHỈ SỬ DỤNG getAdminMessages cho admin
                const response = await chatAPI.getAdminMessages(res.room.id);
        ...
        <div className={`fixed ${position === 'bottom-right' ? 'bottom-4 right-4' : 'bottom-4 left-4'} 
            ...
            <div className={`bg-gradient-to-r ${GREEN_1} text-white px-5 py-4 flex justify-between items-center`}>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-lg font-bold">TravelGo Support (Admin)</h1>
